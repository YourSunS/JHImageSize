...
#import "JHWebImageSize.h"

@implementation JHImageSizeManager

+ (JHImageType)imageTypeOfFilePath:(NSString *)filePath{
    JHImageType type = JHImageTypeUnKnow;
    ...
    else if (bytesArray[0] == 0x52 &&    //R
             bytesArray[1] == 0x49 &&    //I
             bytesArray[2] == 0x46 &&    //F
             bytesArray[3] == 0x46       //F
             ) {
        type = JHImageTypeWebp;
    }
...
{
    if ([filePath checkUrl]) {
        NSLog(@"是网络图");
        return [JHWebImageSize imageSizeWithURL:filePath];
    }
    
    CGSize finalSize = CGSizeZero;
    JHImageType type = [self imageTypeOfFilePath:filePath];
    switch (type) {
        case JHImageTypeJPG:
        {
